			...
			<c:if test="${not fn:contains(clientErrand.status, 3)}">
				...
				<table style="border: 1px solid; width: 1100px;">
					...
					<c:forEach var="applyMember" items="${applyMembers[status.index]}" varStatus="status2">
						<c:if test="${not fn:contains(applyMember.match_status, 2)}"> <!-- 지원자 매칭상태 2 아닌 것만 출력 -->
							...
							<tr>
								...
								<c:choose>
									<c:when test="${fn:contains(clientErrand.status, 0)}">
										<td>매칭대기 <a href="SelectApply?errand_id=${clientErrand.errand_id}&apply_member_id=${applyMember.member_id}"><button>선택하기</button></a>
										</td>
									</c:when>
									...
									<c:when test="${fn:contains(clientErrand.status, 2)}">
										<td>심부름 수행확정 <a href="WriteReviewServiceFromReqErr?apply_member_id=${applyMember.member_id}&errand_title=${clientErrand.title}&errand_id=${clientErrand.errand_id}&errand_ctgr=${clientErrand.errand_ctgr}"><button>후기작성</button></a>
										</td>
									</c:when>
								</c:choose>
							</tr>
